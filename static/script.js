...
var curSym="$";
function getCurSym(sym){var s=(sym||"").toUpperCase();if(s.endsWith(".NS")||s.endsWith(".BO"))return"₹";if(s.endsWith(".L")||s.endsWith(".IL"))return"£";if(s.endsWith(".TO")||s.endsWith(".V"))return"C$";return"$"}
function fmt(n,s){var cs=s||curSym;return cs+Number(n).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}
function fB(n,s){var cs=s||curSym;if(n==null)return"—";if(n>=1e12)return cs+(n/1e12).toFixed(2)+"T";if(n>=1e9)return cs+(n/1e9).toFixed(2)+"B";if(n>=1e6)return cs+(n/1e6).toFixed(1)+"M";return cs+Number(n).toLocaleString()}
...
function selTicker(s){active=s;curSym=getCurSym(s);mainT.textContent=s;renderWL();fetchHist(s);fetchAI(s);fetchFund(s)}
